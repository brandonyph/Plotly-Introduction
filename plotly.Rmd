...
# visual attributes.
p <- palmerpenguins::penguins %>% plot_ly(x = ~bill_length_mm, y = ~body_mass_g)
p
```
...
fig <- plot_ly(x = x, y = y)
fig <- fig %>% layout(xaxis = list(range = c(0, 50)))
fig
```
...
fig <- ds %>% plot_ly(x = ~cut, y = ~clarity, z = ~price, type= "scatter3d", mode = 'markers', marker = list(size = 3))
